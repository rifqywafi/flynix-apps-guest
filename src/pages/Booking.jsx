  ...
  return (
    <ContainerCol>
      ...
      <div className="min-h-[75vh] flex flex-col items-center gap-6 justify-center mb-20">
        ...
        {allFiltersFilled && isVisible.length > 0 ? (
          <>
            {isVisible.map((flight, index) => (
              <div
                key={index}
                className="bg-white border-s-4 border-s-secondary rounded-e-2xl shadow-xl px-6 pb-3 mb-6"
              >
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-3 mt-3">
                      {flight.classes.map((item, idx) => (
                        <span
                          key={idx}
                          className="bg-primary rounded-md text-sm px-2 py-1 text-white"
                        >
                          {item.classType} - Rp
                          {item.totalFare.toLocaleString("id-ID")}
                        </span>
                      ))}
                    </div>
                  </div>
